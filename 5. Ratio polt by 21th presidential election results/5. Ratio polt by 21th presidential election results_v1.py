...
def drawKoreaWithRatioPlots(election_data, ratio_columns, x_col='x', y_col='y', label_col='행정구역'):
    ...
    for _, row in election_data.iterrows():
        ...
        for idx, (ratio, color) in enumerate(zip(ratios, COLORS)):
            ...
            # 비율값(%) 표기
            if ratio > 0.1:  # 너무 작으면 표시 안함
                inset_ax.text(left + ratio / 2, 0.5, f"{int(ratio * 100)}%", ha='center', va='center', fontsize=3,
                              color='white', weight='bold')
            left += ratio

        # 구역명
        inset_ax.text(0.5, 0.1, row[label_col], fontsize=5, ha='center', va='bottom', color='black', weight='bold',
